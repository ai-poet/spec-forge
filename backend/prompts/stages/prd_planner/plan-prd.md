...
- FastAPI: use `app/main.py` or `src/<package>/main.py` as the app entry, split HTTP resources under `routers/` with `APIRouter`, keep shared auth/db/session wiring in `dependencies.py`, separate request/response schemas from persistence/domain models, put orchestration in `services/` or use-case modules, isolate persistence in `repositories/` or `storage/`, and place tests in the existing Python test root.
- HonoJS: use `src/index.ts` as the app assembly point, split resources under `src/routes/*.ts` and mount with `app.route()`, avoid Rails-style controller objects unless the repo already uses them, extract reusable handlers with `hono/factory` when useful, keep business logic in `services/`, DB/client code in `db/` or `lib/`, middleware/env/types separate, and export `AppType` when frontend/RPC clients need typed contracts.
