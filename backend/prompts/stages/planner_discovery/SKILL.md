...
- **Backend ambiguity**: If the requested feature appears to require backend/API/server work but the repository has no existing backend architecture, return `status: ask` and ask whether to add a backend and which stack/runtime to use. Do not silently invent a backend stack.
- **Stack defaults**: If the user leaves stack choices open and the repo does not contradict them, prefer React + Vite with componentized UI, Less Modules, modern large-scale frontend layering, and strong UI/UX usability, visual polish, and fault-tolerant states for frontend/web UI; the existing backend stack with modular route/service/domain/data-access layering, extensibility, maintainability, and performance for backend changes; Electron with main/preload/renderer boundaries for desktop; and Capacitor 7 with shared app/domain/UI layers plus isolated native adapters for mobile.
