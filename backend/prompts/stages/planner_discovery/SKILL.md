...
- **Backend ambiguity**: If the requested feature appears to require backend/API/server work but the repository has no existing backend architecture, return `status: ask` and ask whether to add a backend and which stack/runtime to use. Prefer preset options like "FastAPI (Python + local SQLite prototype)", "HonoJS (TypeScript + local SQLite prototype)", "Supabase (DB/auth/realtime/functions)", and exactly one final "其他（请说明）". Do not silently invent a backend stack.
- **Stack defaults**: If the user leaves stack choices open and the repo does not contradict them, prefer React + Vite under `frontend/**` by default or existing `web/**`, with componentized UI, Less Modules, modern large-scale frontend layering, and strong UI/UX usability, visual polish, and fault-tolerant states for frontend/web UI; the existing backend stack under `backend/**` first for backend changes, or after explicit confirmation a fast backend prototype stack such as FastAPI, HonoJS, or Supabase with clear route/service/domain/data-access, migration, integration, test boundaries, extensibility, maintainability, and performance; Electron with main/preload/renderer boundaries for desktop; and Capacitor 7 with shared app/domain/UI layers plus isolated native adapters for mobile.
