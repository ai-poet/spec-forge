...
28. Cua 不可用时 Web UI 由 Playwright 回退执行（`ui_driver.fallback`）；native UI、未安装 Playwright 或自动化执行失败时记为 `ui_driver.warning`/failed result；只有 UI Tester 汇总出 P0/P1 缺陷时才按 owner 分流，默认回 Coder。
